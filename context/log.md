...
# 2026-02-22（追加：本次对话续记，聚焦 Nano 流畅性 + Web 可视化）

## 1) 本次对话的核心诉求（按时间顺序归并）

1. 先全面梳理项目，再查看 `context/log.md` 理解历史状态。
2. 解决 Nano 上音频预加载卡住/报错问题（用户给出日志：`220.txt`，后续又给 `log/1.txt`、`log/2.txt`）。
3. 调整播报频率：希望“完整一句播报结束后，再进行下一轮场景检测播报”，避免硬件过载导致抢占。
4. 优化 Nano 实机流畅性：已能出声但延迟大、卡顿，要求继续降载并保证可运行。
5. 解释组件职责：用户询问 DashScope 在系统中的用途，并明确提出“尽量不要用到大模型”。
6. 工程协作诉求：多次询问“改了哪些文件便于下载”，并提出上传 GitHub、合并分支（主分支/dev 分支）相关操作需求。
7. 最新问题（当前优先级最高）：
   - `http://localhost:8081` 可视化页面打开后无内容；
   - 画面希望“亮一点”。

## 2) 本轮已确认的技术事实（针对最新 Web 问题）

### 2.1 页面“空白”的直接根因

- `templates/index.html` 页面主体结构完整：包含画布、ASR 状态区、Final 列表、IMU 容器，并加载 `/static/main.js`。
- 但 `static/main.js` 当前是占位文件，仅有简单 `console.log`，没有任何 WS 连接与 DOM 更新逻辑。
- 结论：
  - 前端没有订阅后端数据流，导致页面“看起来没内容/不动”。
  - 根因不在路由缺失，而在前端逻辑未实现。

### 2.2 后端接口是存在且可对接的

已确认 `app_main.py` 中关键接口/协议可用：

1. 页面路由
   - `/`：返回 `templates/index.html`
   - `/static`：静态目录挂载（存在时启用）
2. UI 文本 WS
   - `/ws_ui` 初始发送 `INIT:<json>`，json 含 `{partial, finals}`
   - 推送 `PARTIAL:<text>`、`FINAL:<text>`
3. 视频 WS
   - `/ws/viewer`：推送 JPEG 二进制帧
4. IMU WS
   - `/ws`：推送 IMU JSON 文本
...
1. **优先修复前端而非重构后端**
   - 先重写 `static/main.js`，补齐三路 WS（`/ws_ui`、`/ws/viewer`、`/ws`）与 UI 渲染。
2. **亮度方案优先“低负载”**
   - 考虑 Nano 性能约束，倾向用前端显示层提亮（避免服务端额外图像处理开销）。
   - 若后续实测仍需服务端统一提亮，再追加可配置后处理参数。
3. **流畅性优先策略保持不变**
   - 继续遵循此前已落地的降载方向：限制 viewer FPS、降低质量/分辨率、跳帧透传、减少阻塞路径。

## 4) 关键假设（当前阶段）

1. Nano 与浏览器在同一局域网/可互访，且 `8081` 端口可访问。
2. 浏览器支持标准 WebSocket 与 Canvas（常见 Chromium 环境可满足）。
3. 后端主进程已经在发视频帧与 UI 文本；前端恢复连接后应能立即看到变化。
4. 用户当前最关注的是“能看到内容 + 更流畅”，而非复杂前端动画或 3D IMU 可视化精度。

## 5) 未解决问题（截至本次记录写入时）

1. **`static/main.js` 尚未完成重写落盘**
   - 这是 `localhost:8081` 无内容的主要未完成项。
2. **亮度增强尚未正式落盘**
   - 方案已确定方向（低负载优先），但代码还未提交到文件。
3. **未进行本轮最终联调验证**
   - 尚未完成“修改后启动并在浏览器端验证画面/状态更新/FPS显示”的收口测试。

## 6) 下一步行动（可直接执行）

1. 重写 `static/main.js`（第一优先级）
   - 连接 `/ws_ui`：解析 `INIT/PARTIAL/FINAL`，更新 `#partial`、`#finalList`、`#asrStatus`。
   - 连接 `/ws/viewer`：接收 JPEG 二进制并绘制到 `#canvas`，更新 `#camStatus` 与 `#fps`。
   - 连接 `/ws`：显示 IMU 文本状态与连接状态（可先做轻量可读面板）。
   - 完成“重连/清空”按钮行为（`#btnReconnect`、`#btnClear`）。
2. 提亮画面（第二优先级）
   - 先做前端显示层提亮（低开销）；
   - 如用户仍反馈偏暗，再加服务端可配置提亮参数并评估 CPU 影响。
3. 联调与回归
   - 启动服务后验证 `http://localhost:8081`：
     - 有实时画面；
     - ASR Partial/Final 能刷新；
     - IMU 状态可见；
     - 重连按钮可恢复连接。
4. 验证通过后补充日志
   - 将“实际修改文件清单 + 启动验证结果 + 如有剩余风险”继续追加回 `context/log.md`。

## 7) 本次记录范围说明

- 本段是对“本次对话到当前时刻”的阶段性总结。
- 其中关于 Nano 音频预加载、播报节流、降载方向等，已继承前文同日记录；本段重点更新了最新 Web 可视化故障定位与后续执行路径。
