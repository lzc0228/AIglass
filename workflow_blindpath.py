...
class BlindPathNavigator:
    ...
    def __init__(self, yolo_model=None, obstacle_detector=None):
        ...
        self.last_stable_blind_path_mask = None
        self.blindpath_miss_streak = 0
        ...
        self.BLINDPATH_MIN_AREA_RATIO = float(os.getenv("AIGLASS_BLINDPATH_MIN_AREA_RATIO", "0.004"))
        self.BLINDPATH_MISS_HOLD_FRAMES = int(os.getenv("AIGLASS_BLINDPATH_MISS_HOLD_FRAMES", "2"))
        self.blindpath_miss_streak = 0
        self.last_stable_blind_path_mask = None
    ...
    def process_frame(self, image: np.ndarray) -> ProcessingResult:
        ...
        blind_path_mask = self._stabilize_blind_path_presence(blind_path_mask)
    ...
    def _stabilize_blind_path_presence(self, blind_path_mask: Optional[np.ndarray]) -> Optional[np.ndarray]:
        """在短时漏检时保持盲道连续性，避免导航状态抖动。"""
        area_ratio = 0.0
        if blind_path_mask is not None and blind_path_mask.size > 0:
            try:
                area_ratio = float(np.mean(blind_path_mask > 0))
            except Exception:
                area_ratio = 0.0

        if blind_path_mask is not None and area_ratio >= self.BLINDPATH_MIN_AREA_RATIO:
            stable = (blind_path_mask > 0).astype(np.uint8) * 255
            self.last_stable_blind_path_mask = stable.copy()
            self.blindpath_miss_streak = 0
            return stable

        if (
            self.last_stable_blind_path_mask is not None
            and self.blindpath_miss_streak < self.BLINDPATH_MISS_HOLD_FRAMES
        ):
            self.blindpath_miss_streak += 1
            hold_mask = self.last_stable_blind_path_mask.copy()
            # 连续漏检时轻微收缩，减少旧mask长期滞留风险
            if self.blindpath_miss_streak > 1:
                kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
                hold_mask = cv2.erode(hold_mask, kernel, iterations=1)
            return hold_mask
